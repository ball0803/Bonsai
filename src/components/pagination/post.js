  ...
  return (
    ...
    <Grid container rowSpacing={3} columnSpacing={{ xs: 2, sm: 3, md: 4 }}>
      {posts &&
        posts.map((item) => (
          <Grid item xs={4} >
            <Card className="post" sx={{ maxWidth: 345}} style={{height: '490px', display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <CardActionArea href={item.link} target="_blank" onClick={()=>setDoc(doc(db, 'News', item.id), { viewsIP: arrayUnion(ip) }, { merge: true })}>
                <CardMedia
                  component="img"
                  height="140"
                ...
                <CardContent>
                  <Typography gutterBottom variant="h5" component="div" style={{fontFamily: 'Kanit'}}>
                    {item.header.length > 100 ? `${item.header.slice(0, 70)}...`: item.header}
                  </Typography>
                  {item.summary ?
              ...
              <CardActions>
                ...
                >
                  ...
                  style={{marginLeft: 10}}
                  >
                  {item.viewsIP ? `${nFormatter(item.viewsIP.length)} views | ` : null}
                  <time dateTime={item.date}>{item.date.toDate().toDateString()}</time>
                  </Typography>
                  </ListItem>
                  {item.tag.filter((tag)=>stockList.includes(tag.toUpperCase())).length != 0 ? 
                    <>
